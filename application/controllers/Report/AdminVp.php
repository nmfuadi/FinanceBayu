...
class AdminVp extends AppBase {
	...
	public function notulen() {
		...
		$html = '
										<form action="'.site_url('Report/AdminVp/notulen').'" class="form-horizontal" method="get"> 
                                        <div class="form-body">
		...
		$html .= '			
			...
			<table class="responstable">
				...
				<th colspan="2" width="67%">Subject</th>			
			  </tr>';
		$no = 1;
		if(!empty($jobs)){
			foreach($jobs as $jb){
	...
	public function ext_to() {
		
			
        $this->VIEW_FILE = "Report/Vp/VextData"; // dynamic
		$load_resource = $this->load_resource(); // digawe ngene ikie
		$load_resource['ext'] = 'to';
		$load_resource['judul'] = 'REQUEST TASK LIST OTHER DEPARTMENTS';
		
		$load_resource['jobs'] = $this->M_Admin->get_jobs_by_user_id_ext_to($this->session->userdata('u'));
		$load_resource['tot'] = $this->M_Admin->get_jobs_by_user_id_ext_to_total($this->session->userdata('u'));	
	...
	public function ArchieveJobs() {
        ...
        $this->VIEW_FILE = "Report/admin/adminVp"; // dynamic
		$load_resource = $this->load_resource(); // digawe ngene ikie
		$load_resource['judul'] = 'Archieve Task List';
	 ...
	 public function update($id=null) 
    {
		...
		'ext_st' => set_value('ext_st', $row['ext_st']),
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
			...
			$this->session->set_flashdata('status', 'alert-danger');
            redirect(site_url('Report/AdminVp/update/'.$this->input->post('id', TRUE)));
        } else {
	...
	public function UpdateTaskExt($st=null,$id=null) 
    {
        if(empty($st) or empty($id)){
			 $this->session->set_flashdata('message', 'No Record Data');
			 $this->session->set_flashdata('status', 'alert-danger');
            redirect(site_url('Report/AdminVp'));
		}
	...
	public function ReportTask() {
       ...
       $this->VIEW_FILE = "Report/Vp/VexcelReport"; // dynamic
		...
		$load_resource['data'] = array(
            ...
            'button' => 'View Report',
            'action' => site_url('Report/AdminVp/coba_loop')
			);
