    ...
    function get_data_by_mutation_id($table, $column, $id,$and=null){
     ...
     function total_rows_jurnal($q = NULL) {
        ...
        function SaldoaAwalAccount($mont = null,$year = null,$currancy=null){
            ...
            $sql = "select 
            sum(case when type_mutation='CR' then amount end) as uang_cr,
            sum(case when type_mutation='DB' then amount end) as uang_db,
            sum(case when type_mutation='CR' then original_amount end) as uang_db_ori,
            sum(case when type_mutation='DB' then original_amount end) as uang_db_ori,
            (sum(case when type_mutation='CR' then amount end) - sum(case when type_mutation='DB' then amount end) ) as jml_uang,
